...
import { sendMessage, getMessagesSince } from "@/lib/discord";
import {
...
export async function GET(request: Request) {
  ...
  if (!convo.last_bot_message_id) {
    return Response.json({ skipped: true, reason: "No bot message to check after" });
  }

  // Fetch new messages since our last bot message
  const messages = await getMessagesSince(channelId, convo.last_bot_message_id);

  // Filter to human messages (not bot)
  const userMessages = messages.filter((m) => !m.author.bot);

  if (userMessages.length === 0) {
    return Response.json({ skipped: true, reason: "No new user messages" });
  }
  ...
  // Parse the answer
  const answerValue = parseAnswer(currentQuestion.type, userMsg.content);
  ...
  if (nextQuestion) {
    ...
    // Conversation complete — save to checkins and post summary
